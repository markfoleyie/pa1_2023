...
while incoming_int > 0:                    # keep dividing by 2 until nothing left
    ...
    incoming_int = incoming_int // 2       # '//' is 'integer' division, i.e. yields a whole number only.
                                           # Fill in what has to happen next (make a string of remainder and add it to
...
# Every time we grab a 1 or a 0 (a bit), we are adding a power of two to the overall integer value. Which power of 2? If you grab bits from the right, they are increasing orders of powers of 2. The far right position of the string, or, better said, the last bit in the string (how do you get the last bit??) is 2**0. The next bit 2**1. The next bit 2**2. And so on. If the bit is a ‘1’, then we add that power of 2 to the overall sum; if it is 0 we do nothing.
#
